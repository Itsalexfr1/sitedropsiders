...
function generateQuestions() {
    ...
    const artists_q = [
        ...
        ["Quel artiste est l'auteur de l'album 'Discovery' ?", ["Daft Punk", "Discovery", "Air", "Cassius"], "Daft Punk", "Musique"],
    ...
    const culture_q = [
        ...
        ["Quel DJ est célèbre for son show 'Prismatic' ?", ["Anyma", "Eric Prydz", "Zedd", "Alesso"], "Anyma", "Performance"],
    ...
    const final_q = [
        ...
        ["Quel DJ a mixé pendant l'investiture d'un président français ?", ["Cerrone", "Martin Solveig", "David Guetta", "Bob Sinclar"], "Bob Sinclar", "Performance"],
        ...
        ["Quel DJ porte un masque de 'lapin' ?", ["Aucun", "Deadmau5", "Marshmello", "Vini Vici"], "Aucun", "Bio"],
        ...
        ["Quel DJ est connu for sa résidence 'F*** Me I'm Famous' ?", ["David Guetta", "Bob Sinclar", "Cathy Guetta", "Tiësto"], "David Guetta", "Bio"],
        ...
        ["Quel DJ est connu for son titre 'Toulouse' ?", ["Nicky Romero", "Avicii", "Tiësto", "Afrojack"], "Nicky Romero", "Musique"],
        ...
        ["Quel monument français a accueilli le set de Michael Bibi après sa guérison ?", ["Aucun", "Tour Eiffel", "Louvre", "Arc de Triomphe"], "Aucun", "Événements"],
        ["Quel DJ est célèbre for son show 'Afterlife' au Mexique ?", ["Tale Of Us", "Anyma", "Chris Avantgarde", "Tous"], "Tale Of Us", "Performance"],
    ...
    const all_raw = [...rankings_q, ...festival_q, ...artists_q, ...culture_q, ...final_q];

    const now = new Date().toISOString();
    for (let i = 0; i < all_raw.length; i++) {
        const item = all_raw[i];
        if (!Array.isArray(item)) continue;
        const [q, opts, ans, cat] = item;
        questions.push({
            id: `edm_${i + 1}`,
            type: "QCM",
            question: q,
    ...
    while (questions.length < 200) {
        ...
        const clone = JSON.parse(JSON.stringify(questions[randIndex]));
        clone.id = `edm_extra_${questions.length + 1}`;
...
const path = "edm_questions.json";
fs.writeFileSync(path, JSON.stringify(res, null, 2), 'utf-8');
