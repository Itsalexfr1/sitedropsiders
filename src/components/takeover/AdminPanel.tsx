                                ...
                                <div className="grid grid-cols-1 gap-4 relative z-10">
                                    {editStreams.map((stream, idx) => (
                                        <div key={stream.id} className={`group bg-black/40 border p-6 rounded-2xl transition-all ${editActiveStreamId === stream.id ? 'border-neon-blue shadow-[0_0_20px_rgba(0,255,255,0.1)]' : 'border-white/5 hover:border-white/20'}`}>
                                            ...
                                            {/* Daily Overrides Editor */}
                                            {expandedOverrides === stream.id && (
                                                <div className="mt-6 p-6 bg-black/60 rounded-2xl border border-amber-500/30 space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
                                                    <div className="flex items-center justify-between border-b border-white/5 pb-3">
                                                        <div className="flex items-center gap-2">
                                                            <Calendar className="w-3.5 h-3.5 text-amber-500" />
                                                            <h4 className="text-[10px] font-black text-amber-500 uppercase tracking-[0.2em]">Flux par Jour (Overrides)</h4>
                                                        </div>
                                                        <button 
                                                            onClick={() => {
                                                                const ns = [...editStreams];
                                                                const sIdx = ns.findIndex(s => s.id === stream.id);
                                                                if (!ns[sIdx].dailyOverrides) ns[sIdx].dailyOverrides = [];
                                                                ns[sIdx].dailyOverrides!.push({ day: new Date().toISOString().split('T')[0], youtubeId: '' });
                                                                setEditStreams(ns);
                                                            }}
                                                            className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-lg text-[8px] font-black uppercase tracking-widest hover:bg-amber-500/20 transition-all"
                                                        >
                    ...
                    {adminActiveTab === 'interactif' && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                             ...
                             <div className="bg-red-500/5 border border-red-500/20 rounded-3xl p-6 space-y-4">
                                <h3 className="text-sm font-black text-red-400 uppercase tracking-widest italic border-b border-red-500/10 pb-4 flex items-center gap-3">
                                    <Music className="w-4 h-4" />
                                    Zone Danger — Votes Music
                                </h3>
                                <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider">
                                    Remet tous les compteurs de votes du Top Tracks à zéro. Action irréversible.
                                </p>
                                <button
                                ...
                                >
                                    ...
                                    {isResettingVotes ? 'Reset en cours...' : 'Reset Votes Music'}
                                </button>
                             </div>
