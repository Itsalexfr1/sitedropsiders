            ...
            <div className="flex-1 overflow-hidden relative z-10 min-h-[300px]">
                <AnimatePresence mode="wait">
                    {activeTab === 'random' ? (
                        ...
                        >
                            <div className="flex-1 flex items-center justify-center w-full px-4 md:px-12">
                                <AnimatePresence mode="wait">
                                    {currentQuestion ? (
                                        <motion.div 
                                            key={currentQuestion}
                                            ...
                                            className="text-center space-y-6 md:space-y-10 cursor-pointer w-full"
                                            onClick={() => setSelectedEntry({ artist: artist || 'ARTISTE INCONNU', question: currentQuestion, time: 'NOW' })}
                                        >
                                            <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-neon-cyan/10 border border-neon-cyan/30 rounded-full">
                                                <Sparkles className="w-3.5 h-3.5 text-neon-cyan animate-pulse" />
                                                <span className="text-[9px] md:text-[11px] font-black text-neon-cyan uppercase tracking-[0.3em]">SÉLECTIONNÉE</span>
                                            </div>
                                            <h4 className="text-3xl md:text-6xl lg:text-7xl xl:text-8xl font-display font-black text-white italic tracking-tighter leading-[0.95] drop-shadow-[0_0_50px_rgba(255,255,255,0.15)] uppercase">
                                                "{currentQuestion}"
                                            </h4>
                                        </motion.div>
                                    ) : (
                                        <motion.div 
                                            initial={{ opacity: 0 }}
                                            animate={{ opacity: 1 }}
                                            className="text-center p-12 border-2 border-dashed border-white/5 rounded-[3rem]"
                                        >
                                            <p className="text-gray-600 font-black uppercase tracking-[0.4em] text-[10px] md:text-sm animate-pulse">
                                                {isShuffling ? 'SÉLECTION EN COURS...' : 'PRÊT POUR L\'EXPÉRIENCE ?'}
                                            </p>
                                        </motion.div>
                                    )}
                                </AnimatePresence>
                            ...
                            <div className="flex flex-col items-center gap-8 md:gap-12 w-full mt-8">
                                <button 
                                    onClick={handleShuffle}
                                    disabled={isShuffling}
                                    className="relative group p-1 shrink-0"
                                >
                                    <div className="absolute inset-0 bg-neon-cyan rounded-full blur-xl opacity-20 group-hover:opacity-40 transition-opacity" />
                                    <div className={`w-24 h-24 md:w-36 md:h-36 rounded-full border-4 border-white/10 flex items-center justify-center transition-all ${isShuffling ? 'rotate-180 bg-white/5' : 'hover:scale-110 hover:border-neon-cyan/50 bg-black active:scale-95 shadow-2xl'}`}>
                                        <div className="flex flex-col items-center gap-1.5 md:gap-2">
                                            {isShuffling ? (
                                                <RotateCcw className="w-8 h-8 md:w-12 md:h-12 text-neon-cyan animate-spin" />
                                            ) : (
                                                <>
                                                    <Play className="w-6 h-6 md:w-10 md:h-10 text-white fill-white ml-1.5" />
                                                    <span className="text-[9px] md:text-[11px] font-black text-white uppercase tracking-widest">LANCER</span>
                                                </>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <div className="w-full max-w-sm space-y-3 px-6">
                                    <div className="relative group/input">
                                        <div className="absolute -inset-0.5 bg-gradient-to-r from-transparent via-neon-cyan/20 to-transparent rounded-2xl blur opacity-0 group-focus-within/input:opacity-100 transition-opacity" />
                                        <input 
                                            type="text"
                                            value={artist}
                                            onChange={e => setArtist(e.target.value.toUpperCase())}
                                            placeholder="NOM DE L'ARTISTE..."
                                            className="relative w-full bg-black/60 backdrop-blur-md border border-white/10 rounded-2xl px-6 py-4 text-xs md:text-sm text-white text-center outline-none focus:border-neon-cyan/50 transition-all font-black uppercase tracking-widest placeholder:text-gray-700"
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        >
                            ...
                            <div className="space-y-8 md:space-y-16 w-full px-4">
                                 <div className="space-y-4">
                                     ...
                                     <p className="text-gray-500 font-bold uppercase tracking-[0.3em] text-[9px] md:text-xs mt-3">Question sélectionnée pour l'Interview</p>
                                 </div>

                                 <div className="px-2 md:px-10">
                                     <h2 className="text-3xl md:text-6xl lg:text-7xl xl:text-8xl font-display font-black text-white italic tracking-tighter leading-[1] drop-shadow-[0_0_60px_rgba(255,255,255,0.25)] uppercase">
                                         "{selectedEntry.question}"
                                     </h2>
                                 </div>

                                 <div className="pt-8 md:pt-16 flex items-center justify-center gap-4 md:gap-6">
                                     <div className="h-0.5 w-10 md:w-20 bg-neon-cyan/20" />
                                     <Sparkles className="w-5 h-5 md:w-8 md:h-8 text-white/10" />
                                     <div className="h-0.5 w-10 md:w-20 bg-neon-cyan/20" />
                                 </div>
                            </div>
