...
export function TikTokWidget({ accentColor = 'cyan', resolvedColor, username }: { accentColor?: string, resolvedColor?: string, username?: string }) {
    ...
    // Use IntersectionObserver – fires even if the element is already visible on mount
    useEffect(() => {
    ...
    // Inject TikTok embed script only after blockquote is in the DOM
    useEffect(() => {
        if (!isInView) return;

        // Give React time to render the blockquote before the script processes it
        const timer = setTimeout(() => {
            ...
            script.async = true;
            script.onload = () => setTimeout(() => setIsLoaded(true), 1200);
            script.onerror = () => setIsLoaded(true); // hide skeleton on error too
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            >
                <div className="w-full flex-1 relative rounded-xl overflow-hidden p-[1px] bg-white/5 flex flex-col min-h-[380px]">

                    {/* Skeleton */}
                    {!isLoaded && (
                        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center bg-white rounded-xl">
                    ...
                    <div className="flex-1 bg-white rounded-[11px] overflow-hidden flex flex-col justify-between" style={{ border: `1px solid ${color}20` }}>
