                    ...
                    {latestRecaps.map((item: any, index: number) => (
                        <Link to={`/recaps/${item.id}`} key={item.id} className="block group">
                            <motion.div
                                ...
                                className="h-full group relative aspect-video rounded-lg overflow-hidden cursor-pointer border border-white/10 hover:border-neon-orange/50 transition-all duration-300 shadow-md"
                            >
                                <img
                                    src={item.image}
                                    alt={item.title}
