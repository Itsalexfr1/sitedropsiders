                    ...
                    {latestInterviews.map((item: any, index: number) => (
                        <Link to={`/interviews/${item.id}`} key={item.id} className="block group">
                            <motion.div
                                ...
                                className="h-full group relative aspect-video rounded-2xl overflow-hidden cursor-pointer border border-white/10 hover:border-neon-red/50 transition-all duration-300 shadow-xl bg-black"
                            >
                                <img
                                    src={item.image}
                                    alt={item.title}
