...
export function PlaylistSharing() {
    ...
    // Community mixes tab integration states
    ...
    useEffect(() => {
        if (activeTab === 'uploads' && communityMixes.length === 0) {
    ...
    const handleLikeMix = async (id: string) => {
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-[10px] font-black uppercase tracking-[0.2em] mt-2">
                        {activeTab === 'links' ? 'Tes meilleures playlists Spotify & SoundCloud' : 'Les sets audio uploadés par la communauté'}
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4 flex-wrap z-20">
                    ...
                    {activeTab === 'links' && (
                        <button
                            ...
                            className="px-6 py-4 bg-white text-black rounded-2xl font-black text-[10px] uppercase tracking-widest flex items-center gap-3 hover:bg-neon-red hover:text-white transition-all shadow-lg shadow-black/30"
                        >
                ...
                <div className="space-y-6">
                    {loadingCommunity ? (
                        <div className="py-24 flex flex-col items-center gap-4">
                            ...
                            <p className="text-white/20 text-[10px] font-black uppercase tracking-widest">Chargement de la communauté...</p>
                        </div>
                    ) : communityMixes.length === 0 ? (
                        <div className="py-20 border border-dashed border-white/8 rounded-[36px] flex flex-col items-center gap-4 text-center">
                            <Disc3 className="w-12 h-12 text-white/25 animate-spin-slow" />
                            <p className="text-white/30 font-black uppercase tracking-widest text-sm">Aucun mix public pour le moment</p>
                        </div>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {communityMixes.map((mix, idx) => (
                                <motion.div
                                    key={mix.id}
                                    initial={{ opacity: 0, y: 20 }}
                                    animate={{ opacity: 1, y: 0 }}
                                    transition={{ delay: idx * 0.05 }}
                                    className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 space-y-5 hover:border-white/20 transition-all flex flex-col justify-between"
                                >
                                    <div className="space-y-4">
                                        <div className="flex justify-between items-start">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2 flex-wrap">
                                                    <span className="text-[9px] font-black uppercase tracking-widest px-2.5 py-0.5 rounded-full border text-neon-purple border-neon-purple/20 bg-neon-purple/10">
                                                ...
                                                <h3 className="text-xl font-display font-black text-white italic uppercase tracking-tight mt-1">{mix.title}</h3>
                                                {mix.description && <p className="text-gray-500 text-[11px] leading-relaxed mt-1">"{mix.description}"</p>}
                                            </div>
                                            ...
                                            >
                                                <Heart className={`w-5 h-5 ${likedMixes.has(mix.id) ? 'fill-current' : ''}`} />
                                                <span className="text-[8px] font-black">{mix.likes || 0}</span>
                                            </button>
                                        </div>
                                    </div>
